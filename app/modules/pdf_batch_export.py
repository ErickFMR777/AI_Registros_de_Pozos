...
def create_pdf_batch_report(wells_data, language='es'):
    ...
    from .pdf_export import generate_8track_figure, _pdf_t
    ...
    elements.append(Paragraph(t('report_title'), main_title_style))
    elements.append(Paragraph({
        'es': 'REPORTE CONSOLIDADO DE POZOS',
        'en': 'CONSOLIDATED WELL REPORT',
        'fr': 'RAPPORT CONSOLIDÉ DE PUITS'
    }.get(language, 'REPORTE CONSOLIDADO DE POZOS'), main_title_style))
    elements.append(Spacer(1, 0.2*inch))
    elements.append(Paragraph(f"Total: {len(wells_data)} { {'es':'Pozos','en':'Wells','fr':'Puits'}.get(language,'Pozos') }", ParagraphStyle(
    ...
    # Resumen general
    elements.append(Paragraph({
        'es': 'RESUMEN GENERAL DE POZOS',
        'en': 'GENERAL WELL SUMMARY',
        'fr': 'RÉSUMÉ GÉNÉRAL DES PUITS'
    }.get(language, 'RESUMEN GENERAL DE POZOS'), heading_style))
    elements.append(Spacer(1, 0.1*inch))
    
    summary_data = [[
        {'es': 'POZO', 'en': 'WELL', 'fr': 'PUITS'}.get(language, 'POZO'),
        {'es': 'PROF. INI. (FT)', 'en': 'START DEPTH (FT)', 'fr': 'PROF. DÉBUT (FT)'}.get(language, 'PROF. INI. (FT)'),
        {'es': 'PROF. FIN. (FT)', 'en': 'END DEPTH (FT)', 'fr': 'PROF. FIN (FT)'}.get(language, 'PROF. FIN. (FT)'),
        t('samples'),
        {'es': 'MATRIZ', 'en': 'MATRIX', 'fr': 'MATRICE'}.get(language, 'MATRIZ'),
        'NET PAY (%)'
    ]]
    ...
    for well in wells_data:
        ...
        samples = len(df)
        matrix = config.get('DOMINANT_MATRIX', t('na'))
    ...
    for well_idx, well_data in enumerate(wells_data):
        ...
        # ===== TÍTULO DEL POZO =====
        elements.append(Paragraph(f"{t('well').upper()}: {well_name.upper()}", well_title_style))
        ...
        calc_data = [
            ...
            [t('archie_a'), f"{config.get('A', 0):.3f}"],
            [t('archie_m'), f"{config.get('M', 0):.3f}"],
            [t('archie_n'), f"{config.get('N', 0):.3f}"],
            [t('rw'), f"{config.get('RW', 0):.4f}"],
    ...
    elements.append(Paragraph(
        f"<i>{ {'es':'Reporte Consolidado generado','en':'Consolidated report generated','fr':'Rapport consolidé généré'}.get(language, 'Reporte Consolidado generado') }: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}</i>",
